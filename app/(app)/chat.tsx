...
import { TextInput, IconButton, Surface, Text, ActivityIndicator, FAB, Menu } from 'react-native-paper';
...
import type { FileInfo } from '@/src/services/fileService';
...
export default function Chat() {
    ...
    const {isPending, mutate} = useMutation({
        ...
        onError: () => {
            setMessages(prev =>
                prev.filter(msg => msg.id !== 'generating').concat({
                    id: Date.now().toString(),
            ...
            case 'document':
                return (
                    <View style={styles.documentContainer}>
                        <IconButton icon={file.type === 'pdf' ? 'file-pdf-box' : 'file-document'} size={24} />
                        <View style={styles.documentInfo}>
                            <Text numberOfLines={1} style={styles.documentName}>{file.name}</Text>
                            {file.size && <Text style={styles.documentSize}>{fileService.formatFileSize(file.size)}</Text>}
                        </View>
                    </View>
                );
            default:
    ...
    const renderMessage = ({ item }: { item: ChatMessage }) => (
        ...
        >
            <View style={styles.messageContent}>
                ...
                {item.text && !item.isGenerating && <Text style={styles.messageText}>{item.text}</Text>}
                {item.isGenerating && (
                    <ActivityIndicator size={20} color="#666" style={styles.generatingIndicator} />
                )}
            </View>
        </Surface>
    );
